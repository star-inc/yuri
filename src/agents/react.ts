 ...
 * @param {AgentState} state The current agent state.
 * @param {Record<string, StructuredToolInterface>} toolMapOverride
 *     Tool map to use.
 * @return {Promise} Tool execution results.
 */
async function callTools(
    state: AgentState,
    toolMapOverride: Record<string, StructuredToolInterface> = baseToolMap,
):
    Promise<{
    ...
    const results = await Promise.all(
        uniqueCalls.map(async (call) => {
            ...
            const tool = toolMapOverride[call.name];
...
export function createReActAgent(config: ReActAgentConfig) {
    ...
    const allToolMap: Record<string, StructuredToolInterface> = {
        ...baseToolMap,
        ...Object.fromEntries(
            (config.extraTools ?? []).map((t) => [t.name, t]),
        ),
    };
    ...
    const workflow = new StateGraph(AgentStateAnnotation)
        ...
        .addNode("tools", (state: AgentState) => callTools(state, allToolMap))
        .addEdge(START, "agent")
